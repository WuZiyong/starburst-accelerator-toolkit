...
def _remote_probe(profile: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not ssh_host or not expected or hostname() == str(expected):
        return None

    backend = str(profile.get("backend"))
    runtime = profile.get("runtime", {})
    if backend == "dcu":
        root = str(runtime.get("dtk_root") or "/opt/dtk")
        pci = str(profile.get("hardware", {}).get("pci_id") or "1d94:54b7")
        shell = (
            f'source "{root}/env.sh"\n'
            'export PATH="/sbin:/usr/sbin:$PATH"\n'
            'echo HOST=$(hostname)\n'
            f'"{root}/bin/hy-smi" --showmeminfo vram\n'
            f'echo PCI_COUNT=$(/sbin/lspci -nn | grep -c "{pci}" || true)'
        )
    elif backend == "cuda":
        shell = "echo HOST=$(hostname)\nnvidia-smi -L\nnvidia-smi --query-gpu=name,memory.total --format=csv,noheader"
    elif backend == "rocm":
        root = str(runtime.get("rocm_root") or "/opt/rocm")
        shell = f'echo HOST=$(hostname)\n"{root}/bin/rocm-smi"'
    else:
        return None

    try:
        proc = subprocess.run(
            ["ssh", "-o", "BatchMode=yes", str(ssh_host), "bash", "-s"],
    ...
    result: dict[str, Any] = {
        "check": "remote_hardware",
        "status": "PASS" if rc == 0 else "FAIL",
        "host": str(ssh_host),
        "output": out.strip(),
    }
    if backend == "dcu" and rc == 0:
        match = re.search(r"PCI_COUNT=(\d+)", out)
        visible = int(match.group(1)) if match else None
        ...
        if visible is not None and configured is not None and visible != int(configured):
            result["status"] = "WARN"
    return result
...
def doctor(profile: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    remote = _remote_probe(profile)
    if remote is not None:
        checks.append(remote)
    else:
        checks.extend(get_backend(str(profile["backend"])).doctor(profile))
    ...
    if scheduler.get("type") == "slurm":
        ...
        if node:
            ...
            state = next(
                (part.split("=", 1)[1] for part in out.replace("\n", " ").split() if part.startswith("State=")),
                None,
            )
